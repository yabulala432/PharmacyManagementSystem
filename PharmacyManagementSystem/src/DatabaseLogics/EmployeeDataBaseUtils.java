...
public class EmployeeDataBaseUtils extends DataBaseUtils {
    ...
    private enum Queries {
        ...
        UPDATE_EMPLOYEE_Hours("UPDATE Employee SET Hours = ?, Wage = ? WHERE Username = ?"),
        ...
        DELETE_FROM_EMPLOYEE("DELETE FROM Employee WHERE Username = ?");
    ...
    public static boolean updateHoursAndWage(String username, int hours, double wage) {
        try {
            preparedStatement = getConnection().prepareStatement(
                    Queries.UPDATE_EMPLOYEE_Hours.getQuery()
            );
            preparedStatement.setInt(1, hours);
            preparedStatement.setDouble(2, wage);
    ...
    public static int deleteEmployee(String username) {
        try {
            String sql = Queries.DELETE_FROM_EMPLOYEE.getQuery();
            preparedStatement = getConnection().prepareStatement(sql);
            preparedStatement.setString(1, username);
            int rowsAffected = preparedStatement.executeUpdate();
            preparedStatement.close();
